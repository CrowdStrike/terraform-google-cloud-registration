...
variable "registration_id" {
  type        = string
  description = "Unique registration ID returned by CrowdStrike Registration API, used as the allowed audience for OIDC tokens"
...
variable "service_account_unique_id" {
  type        = string
  description = "Numeric unique ID of CrowdStrike's shared service account, used as the subject assertion for OIDC federation"

  validation {
    condition     = can(regex("^[0-9]+$", var.service_account_unique_id))
